...
One codebase, two Modal deployments (gpt-5-nano and Gemini Flash-Lite),
selected via the ``BASELINE_MODEL`` environment variable. See the README.

Modules:
  config               — env-driven config; picks the model + credential pair
  webhook_app          — FastAPI app factory: verify, dedupe, spawn, ACK fast
  worker               — async event processing: fetch facts → predict → submit
