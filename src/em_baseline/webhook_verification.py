...
verifier, copied in (rather than installed from PyPI) so the baseline is
self-contained and has zero runtime dependencies for verification.
...
Usage::
    ...
    from em_baseline import verify_webhook, WebhookVerificationError
...
def verify_webhook(
    *,
    ...
    if abs(current - timestamp) > tolerance_seconds:
        raise WebhookVerificationError(f"Webhook-Timestamp outside {tolerance_seconds}s tolerance")
